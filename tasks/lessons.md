...
- **[L1](#l1--a-guard-is-only-correct-at-the-altitude-it-was-written-for)** A guard is only correct at the altitude it was written for
- **[L2](#l2--a-default-that-fires-on-missing-must-not-fire-on-misparsed)** A default that fires on "missing" must not fire on "misparsed"
- **[L3](#l3--leaving-a-compound-unit-out-of-the-table-does-not-make-it-refuse)** Leaving a compound unit out of the table does not make it refuse
- **[L4](#l4--green-is-not-the-same-as-right-a-check-can-pin-the-bug)** Green is not the same as right; a check can pin the bug
- **[L5](#l5--measure-the-before-by-running-it-not-by-remembering-it)** Measure the "before" by running it, not by remembering it
- **[L6](#l6--a-bare-except-around-an-unverified-api-reports-your-data-is-broken)** A bare `except` around an unverified API reports "your data is broken"
- **[L7](#l7--ask-what-the-number-means-before-reporting-a-count)** Ask what the number means before reporting a count
- **[L8](#l8--a-measurement-that-kills-its-subject-will-eventually-corrupt-its-subject)** A measurement that kills its subject will eventually corrupt its subject
- **[L9](#l9--pkill--f-matches-the-command-line-that-invoked-it)** `pkill -f` matches the command line that invoked it
- **[L10](#l10--it-isnt-there-is-only-evidence-once-the-path-is-confirmed)** "It isn't there" is only evidence once the path is confirmed
- **[L11](#l11--a-substring-check-over-a-whole-file-matches-the-comment-explaining-it)** A substring check over a whole file matches the comment explaining it
- **[L12](#l12--a-loaded-page-is-not-text-and-a-non-empty-list-is-not-non-empty-content)** A loaded page is not text, and a non-empty list is not non-empty content
- **[L13](#l13--a-prescribed-fix-names-a-mechanism-check-the-mechanism-exists-before-applying-it)** A prescribed fix names a mechanism; check the mechanism exists before applying it
- **[L14](#l14--a-specification-names-a-framework-check-the-framework-is-still-in-the-repo)** A specification names a framework; check the framework is still in the repo
- **[L15](#l15--a-test-that-builds-its-own-world-never-sees-what-the-repository-put-in-the-real-one)** A test that builds its own world never sees what the repository put in the real one
- **[L16](#l16--a-lookup-table-keyed-differently-from-the-lookup-has-silent-holes)** A lookup table keyed differently from the lookup has silent holes
- **[L17](#l17--a-url-that-authenticates-is-a-password-with-a-scheme-on-the-front)** A URL that authenticates is a password with a scheme on the front
- **[L18](#l18--a-shell-heredoc-eats-a-backslash-level-and-it-can-write-a-nul-into-a-document)** A shell heredoc eats a backslash level, and it can write a NUL into a document
- **[L19](#l19--a-model-given-an-empty-document-does-not-report-an-empty-document)** A model given an empty document does not report an empty document
- **[L20](#l20--a-substring-search-finds-only-the-words-the-document-literally-contains)** A substring search finds only the words the document literally contains
- **[L21](#l21--a-fallback-must-not-share-a-shutdown-signal-with-the-thing-it-is-a-fallback-for)** A fallback must not share a shutdown signal with the thing it is a fallback for
- **[L22](#l22--a-new-persistent-file-makes-every-existing-harness-a-writer-to-it)** A new persistent file makes every existing harness a writer to it
- **[L23](#l23--a-table-selected-at-import-makes-every-harness-a-test-of-the-wrong-platform)** A table selected at import makes every harness a test of the wrong platform
- **[L24](#l24--a-guard-that-never-fires-makes-the-checks-after-it-pass-while-testing-nothing)** A guard that never fires makes the checks after it pass while testing nothing
- **[L25](#l25--a-textual-scan-of-a-well-commented-file-reads-the-prose-not-the-code)** A textual scan of a well-commented file reads the prose, not the code
- **[L26](#l26--look-for-what-the-machine-has-already-recorded-before-inventing-a-corpus)** Look for what the machine has already recorded, before inventing a corpus
- **[L27](#l27--a-models-capability-list-is-a-claim-the-real-prompt-is-the-test)** A model's capability list is a claim; the real prompt is the test
- **[L28](#l28--a-windows-path-in-a-python-string-is-an-escape-sequence-and-the-module-stops-importing)** A Windows path in a Python string is an escape sequence, and the module stops importing
- **[L29](#l29--a-flag-that-answers-a-coarser-question-than-the-one-being-asked)** A flag that answers a coarser question than the one being asked
- **[L30](#l30---the-gate-that-rejected-the-answer-for-a-character-nobody-can-see)** The gate that rejected the answer for a character nobody can see
- **[L31](#l31---a-second-way-to-do-something-he-could-already-do)** A second way to do something he could already do
- **[L32](#l32---reporting-a-gap-honestly-is-not-the-same-as-closing-it)** Reporting a gap honestly is not the same as closing it
- **[L33](#l33---ask-which-job-actually-needs-the-model-not-how-to-make-the-model-cheaper)** Ask which job actually needs the model, not how to make the model cheaper
- **[L34](#l34---a-character-class-is-a-claim-about-the-world-and-hyphens-are-punctuation)** A character class is a claim about the world, and hyphens are punctuation
- **[L35](#l35---run-it-once-out-loud-before-you-call-it-done)** Run it once, out loud, before you call it done
- **[L36](#l36---do-not-lower-the-threshold-until-the-wrong-answer-passes)** Do not lower the threshold until the wrong answer passes
- **[L37](#l37---a-docstring-that-disagrees-with-its-code-is-a-bug-report-nobody-has-read-yet)** A docstring that disagrees with its code is a bug report nobody has read yet
- **[L38](#l38---fixing-the-thing-on-the-board-would-not-have-fixed-the-bug)** Fixing the thing on the board would not have fixed the bug
- **[L39](#l39---check-that-a-new-fixture-would-have-failed-before)** Check that a new fixture would have failed before
- **[L40](#l40---fix-both-halves-of-a-safety-pair-or-neither-is-fixed)** Fix both halves of a safety pair, or neither is fixed
- **[L41](#l41---read-the-log-before-asking-what-is-wrong-and-read-it-for-silence-as-well-as-errors)** Read the log before asking what is wrong, and read it for SILENCE as well as errors
- **[L42](#l42---a-warning-logged-and-not-surfaced-is-a-bug-with-a-paper-trail)** A warning logged and not surfaced is a bug with a paper trail
- **[L43](#l43---set-the-limit-on-the-activity-not-on-the-process)** Set the limit on the ACTIVITY, not on the process
- **[L44](#l44---cost-the-cheap-thing-before-optimising-the-expensive-one)** Cost the cheap thing before optimising the expensive one
- **[L45](#l45---never-lose-what-was-heard-unless-the-repair-is-worse-than-the-loss)** Never lose what was heard, unless the repair is worse than the loss
- **[L46](#l46---check-the-brief-against-the-code-before-building-to-it)** Check the brief against the code before building to it
- **[L47](#l47---if-a-number-would-settle-the-question-check-whether-anything-ever-wrote-it-down)** If a number would settle the question, check whether anything ever wrote it down
- **[L48](#l48---delay-the-decision-do-not-delete-the-data)** Delay the decision, do not delete the data
- **[L49](#l49---a-partial-sweep-is-how-a-green-suite-hides-a-break)** A partial sweep is how a green suite hides a break
- **[L50](#l50---ask-how-old-the-running-process-is-before-you-believe-a-session)** Ask how old the running process is before you believe a session
- **[L51](#l51---a-check-about-where-a-path-resolves-has-no-business-writing-to-it)** A check about where a path resolves has no business writing to it
- **[L52](#l52---two-weak-models-agreeing-is-not-evidence-go-back-to-the-recording)** Two weak models agreeing is not evidence; go back to the recording
- **[L53](#l53---do-not-price-an-option-you-have-not-read-the-code-for)** Do not price an option you have not read the code for
...
## L53 - Do not price an option you have not read the code for

Offering LB a choice about the wake word, I described one option as cutting the microphone's
open time after a false wake "from 25 seconds to about 3". He chose it, partly on that number.

The number was wrong. `engine/run_voice.py::turn_finished` already refuses to extend
`conversation_s` on a turn that heard nothing - the guard has been there since D44 and its
docstring says exactly why. A false wake never held the microphone for 25 seconds. What it
actually costs is the greeting plus a second capture attempt: about seven seconds of him being
audibly awake, and the spoken "What's up LB?" is the part LB hears from another room.

The fix that shipped is therefore a different fix from the one described - `greet_threshold`,
which gates the VOICE - and it happens to be the better one, which is luck rather than method.

**An option presented to a user is a claim about the code, and it carries the same burden of
proof as a docstring.** I had read `turn_finished` earlier in the same session; I wrote the
option from memory of the config comment instead, and the config comment was about a different
mechanism. Read the function again before quoting a number from it.
