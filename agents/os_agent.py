...
Reading `Get-ChildItem -Path 'C:\Users\user\OneDrive\Desktop' -Filter *.stl` out loud
gives "get dash child item dash path c colon backslash users backslash user backslash one
...
[Action: AI triggers execute_terminal_command with args: command="Get-ChildItem -Path 'C:\Users\user\OneDrive\Desktop' -Filter *.stl -File | Select-Object Name, Length"]
...
def propose_launch(app: str, spoken: str) -> Response:
    ...
    `Get-ChildItem -Path 'C:\Users\user\OneDrive\Desktop'` aloud is unusable as a question. But "Want me
